...
export function Past() {
  ...
  const getData = () => {
    if (
      localStorage.getItem("pastMeeting") !== null &&
      localStorage.getItem("pastMeeting") !== "undefined"
    ) {
      setHideLoading(true);
      setMeetingList(JSON.parse(localStorage.getItem("pastMeeting")));
    } else {
      setLoading(true);
    }
    axios
      .get(
        `https://meet-production-52c7.up.railway.app/api/booking/pastmeeting?id=${user.id}`
      )
      .then(
        (response) => (
          setMeetingList(response.data),
          localStorage.setItem("pastMeeting", JSON.stringify(response.data))
        )
      )
